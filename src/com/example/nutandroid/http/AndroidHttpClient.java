...
public final class AndroidHttpClient implements HttpClient
{
	...
	private static final NutLogger logger = NutLogger.getLogger(AndroidHttpClient.class);

	// Gzip of data shorter than this probably won't be worthwhile
	public static long DEFAULT_SYNC_MIN_GZIP_BYTES = 256;

	private static final String userAgent = "ZSMART_MOBILE_FRAMEWORK";
	...
	private static final int connectionsPerRoute = 125;
	...
	public static AndroidHttpClient newInstance(Context context)
	{
		...
		HttpConnectionParamBean cBean = new HttpConnectionParamBean(params);
		cBean.setConnectionTimeout(connectionTimeout);
		cBean.setSoTimeout(soTimeout);
		cBean.setSocketBufferSize(socketBufferSize);
		// Turn off stale checking. Our connections break all the time anyway,
		// and it's not worth it to pay the penalty of checking every time.
		cBean.setStaleCheckingEnabled(false);

		ConnManagerParamBean conManagerBean = new ConnManagerParamBean(params);
		ConnPerRouteBean connPerRouteBean = new ConnPerRouteBean(connectionsPerRoute);
		conManagerBean.setConnectionsPerRoute(connPerRouteBean);
		conManagerBean.setMaxTotalConnections(maxTotalConnections);

		HttpProtocolParamBean pBean = new HttpProtocolParamBean(params);
		pBean.setVersion(HttpVersion.HTTP_1_1);
		pBean.setContentCharset("UTF-8");
		pBean.setHttpElementCharset("UTF-8");
		pBean.setUserAgent(userAgent);
		...
		schemeRegistry.register(new Scheme("https", SSLCertificateSocketFactory.getHttpSocketFactory(30 * 1000, sessionCache), 443));
	...
	{
		return newInstance(null /* session cache */);
	}
		...
		this.delegate = new DefaultHttpClient(ccm, params)
		{
			...
			protected HttpRequestRetryHandler createHttpRequestRetryHandler()
			{
				return new DefaultHttpRequestRetryHandler()
				{
					...
					{
						boolean ifRetry = super.retryRequest(exception, executionCount, context);
						logger.warn("exception whether retry {} [{}]", exception.getClass(), exception.getMessage());
						logger.debug("retry:{},exception:{},count:{},threadName:{}", ifRetry, exception, executionCount, Thread.currentThread().getName());
						return ifRetry;
					}
				};
			}
